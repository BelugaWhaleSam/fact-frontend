...
export default function Owner() {
  ...
  function handleMint(recipient: any) {
    const apiCall = () => {
      return axios.get(
        "http://localhost:3000/authorize_provider?address=" + recipient,
        {
          headers: {
            from: "owner",
          },
  ...
  const onFinish = async (values: any) => {
    ...
    for (let i = 0; i < researcherAccounts.length; i++) {
      if (values[researcherAccounts[i]["account_name"] + "type"] == undefined) {
        let res = await handleMintResearcher(
          researcherAccounts[i]["address"],
          researcherAccounts[i]["access_types"]
        );
        tracker.push(res);
      } else {
        let res = await handleMintResearcher(
          researcherAccounts[i]["address"],
          values[researcherAccounts[i]["account_name"] + "type"]
        );
        tracker.push(res);
      }
    }
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios.get("http://localhost:3000/reset_public_keys", {
          headers: {
            "Content-Type": "application/json",
            from: "owner",
          },
    ...
    axios.get("http://localhost:3000/all_access_policies").then((res) => {
      console.log(res.data);
      let temp: any = [];
      let seen = new Set();
      ...
      {hospitalAccounts.length > 0 && researcherAccounts.length > 0 && (
        ...
        >
          ...
          <Form
            ...
            onFinish={onFinish}
          >
            <Form.Item label="Data Providers" name="recipient1">
            ...
            >
              <Select
                ...
                defaultValue={Array.from(
                  new Set(options.map((val: any) => val["label"]))
                )}
            ...
            <Form.Item label="Data Analysts" name="recipient4">
              {researcherAccounts.map((val) => (
                ...
                >
                  <Form.Item
                    ...
                    style={{ flex: 1 }}
                    name={[
                      val["account_name"] + stringify(val["account_id"]),
                      "first",
                    ]}
                  >
                  ...
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Please select access policy",
                      },
                    ]}
                  >
                    <Select
            ...
            <div style={{ width: "100%", fontWeight: "bold" }}>
              {
                <TextArea
                  rows={5}
                  style={{ color: "black" }}
                  defaultValue={mint.map((token) => JSON.stringify(token))}
                />
              }
            </div>
          )}
